...
const regex4 = new RegExp(/^((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])/);
const regex6 = new RegExp(/^(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))/);
...
class IP {
  ...
  getFull6() {
    ...
    if (this.is4) {
      ...
      const a = [(m[1] * 1).toString(16), (m[2] * 1).toString(16), (m[3] * 1).toString(16), (m[4] * 1).toString(16)];
      return '0000:0000:0000:0000:0000:ffff:' + (a[0].length < 2 ? '0' + a[0] : a[0]) + (a[1].length < 2 ? '0' + a[1] : a[1]) + ':' + (a[2].length < 2 ? '0' + a[2] : a[2]) + (a[3].length < 2 ? '0' + a[3] : a[3]);
    } else {
      ips = ips.replace(/^:|:$/g, '');
      let ipv6 = ips.split(':');
  ...
  getRange4() {
    ...
    if (!/\d+\.\d+\.\d+\.\d+/.test(part[1])) {
      blocks = ('1'.repeat(parseInt(part[1], 10)) + '0'.repeat(32 - parseInt(part[1], 10))).match(/.{1,8}/g);
              ...
              for (let i6 = cis[5]; i6 <= cie[5]; i6++) {
                for (let i7 = cis[6]; i7 <= cie[6]; i7++) {
                  for (let i8 = cis[7]; i8 <= cie[7]; i8++) {
                    const ip = i1.toString(16).padStart(4, '0') + ':' + i2.toString(16).padStart(4, '0') + ':' + i3.toString(16).padStart(4, '0') + ':' + i4.toString(16).padStart(4, '0') + ':' + i5.toString(16).padStart(4, '0') + ':' + i6.toString(16).padStart(4, '0') + ':' + i7.toString(16).padStart(4, '0') + ':' + i8.toString(16).padStart(4, '0');
                    ips.push(ip);
